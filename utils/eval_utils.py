...
from utils.get_iou import official_get_iou_func
...
def eval_ocr(task, generator, models, sample, **kwargs):
    ...
    hyps, refs, results = [], [], []
    for i, sample_id in enumerate(sample["id"].tolist()):
        decode_tokens = decode_fn(
            gen_out[i][0]["tokens"], task.tgt_dict, task.bpe, generator
        ).strip()
...
def merge_results(task, cfg, logger, score_cnt, score_sum, results, use_csv):
    if task.cfg._name == "image_gen":
        ...
        if cfg.distributed_training.distributed_world_size == 1 or dist.get_rank() == 0:
            ...
            df = DataFrame(gather_results)
            output_path = (
                output_path.replace(".json", ".csv") if use_csv else output_path
            )
